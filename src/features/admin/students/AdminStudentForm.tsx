...
const AdminStudentForm: React.FC = () => {
  ...
  const { handleSubmit, reset, control, register, setValue, formState } = form;
  const { isSubmitting, errors } = formState;

  // Move useWatch to top level
  ...
  useEffect(() => {
    if (studentData && isEditMode) {
      // Handle slotId - could be string or object
      let slotIdValue = "";
      if (typeof studentData.slotId === "string") {
        slotIdValue = studentData.slotId;
      } else if (studentData.slotId && typeof studentData.slotId === "object") {
        slotIdValue = studentData.slotId._id || "";
      }
        ...
        <div className="min-h-screen bg-[#f8fafc] p-4 sm:p-6 md:p-8 animate-in fade-in duration-500">
          <div className="max-w-6xl mx-auto space-y-8">
            ...
            <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
              ...
              <Badge
                variant="outline"
                className="rounded-xl px-4 py-2 bg-white border-slate-200 text-slate-500 gap-2 font-bold uppercase text-[10px] tracking-widest"
              >
              ...
              <div className="lg:col-span-8 space-y-6">
                ...
                <Card className="border-none shadow-sm ring-1 ring-slate-200 rounded-[24px] overflow-hidden bg-white transition-all hover:shadow-md">
                  <CardContent className="p-6 md:p-8 space-y-6">
                    ...
                    {/* FIXED: LibraryInfoSection expects form prop */}
                    <LibraryInfoSection
                      form={form}
                      slotsData={slotsData}
                ...
                <Card className="border-none shadow-sm ring-1 ring-slate-200 rounded-[24px] overflow-hidden bg-white transition-all hover:shadow-md">
                  <CardContent className="p-6 md:p-8 space-y-6">
                    ...
                    {/* FIXED: AdditionalInfoSection expects individual props */}
                ...
                <Card className="border-none shadow-xl ring-1 ring-slate-900 bg-slate-900 rounded-[24px] overflow-hidden">
                  <CardContent className="p-6 space-y-4">
                    ...
                    <Button
                      ...
                      className="w-full h-14 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-black uppercase text-[11px] tracking-widest shadow-lg shadow-blue-900/20 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {isSubmitting ? (
